...
\label{tab:crossover-validation-arch}
...
\noindent Empirical calibration (time/size).\\

\[
\begin{aligned}
 C_{\text{proof}} &= 0.00956\,\text{s/item}, &\quad C_{\text{storage}} &= 783\,\text{bytes/proof}, \\
 C_{\text{verify}} &= 0.02488\,\text{s/proof}, &\quad C_{\text{setup}} &= 0.0713\,\text{s}, \\
 C_{\text{fold}} &= 0\,\text{s/level}, &\quad C_{\text{const}} &= 0.01\,\text{s}, \\
 && \log &\equiv \log_{2}.
\end{aligned}
\]
...
\noindent With calibration batch size $b=10$ we have $n_{\text{proofs}}=\lceil 171/10 \rceil = 18$.

    \begin{align}
    C_{\text{standard-time}}(171) &= 171\cdot 0.00956\; +\; 18\cdot 0.02488\; +\; 0.0713 \;\approx\; 2.153\;\text{s} \\
    C_{\text{recursive-time}}(171) &= 171\cdot 0.00956\; +\; 0\cdot \log_{2}(18)\; +\; 0.01 \;\approx\; 1.644\;\text{s} \\
    \text{Efficiency Ratio (time)} &= \dfrac{2.153}{1.644} \;\approx\; 1.31
    \end{align}

\begin{comment}
\paragraph{Interpreting efficiency metrics}
\begin{itemize}
  \item \textbf{Storage efficiency}: Final proof size is constant under recursion. With standard per-proof size 783\,B vs. measured $\approx$520\,B final proof, size-efficiency grows roughly as $\tfrac{783}{520}\,n_{\text{proofs}} \approx 1.51\,n_{\text{proofs}}$.
  \item \textbf{Memory efficiency}: Recursive peak memory scales sub-linearly (empirically $\propto n^{0.7}$) vs. linear for standard, benefiting RAM-constrained devices already at small $n$.
  \item \textbf{Proving efficiency}: Folding/setup overheads dominate initially; advantages appear only once these are amortized (large $n$).
  \item \textbf{Overall}: Combination of the above; around $n\!\approx\!171$ the total cost crosses in favor of recursion.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{img/crossover_point_overview.png}
    \caption{Overview plot of the crossover point. The curves show total cost for standard vs. recursive SNARKs across data sizes; the green band indicates the 95\% confidence region for the crossover (153--188 items) and the dashed line marks the point estimate at 171 items. Shaded areas denote superiority regions.}
    \label{fig:crossover-overview}
\end{figure}

\begin{comment}
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{img/theoretical_crossover_analysis.png}
    \caption[Decomposition of the crossover]{Decomposition of the crossover: total cost, proving components, storage and relative efficiency ratio. This multi-panel view explains which factors dominate at different scales.}
    \label{fig:crossover-theory}
\end{figure}

\paragraph{Scientific Analysis (Why/How)}
The calibrated model and generated plots jointly explain \emph{why} recursive SNARKs become superior and \emph{under which conditions}:
\begin{itemize}
    \item \textbf{Main crossover (overall)} occurs at $n^* = 171$ items (95\% CI: [153, 188]). With the calibrated times, the measured efficiency ratio at $n^*$ is $\approx 1.31\times$ (time), i.e., a clear advantage that grows beyond the threshold.
    \item \textbf{Storage crossover} is \emph{immediate} with the measured final recursive proof size ($\approx$520\,B) versus 783\,B per standard proof; since $\lceil 520/783 \rceil = 1$, the constant-size proof is already smaller after the first batch.
    \item \textbf{Memory crossover} is immediate (at $n=1$) because the recursive path scales sub-linearly ($\propto n^{0.7}$ in our calibration) whereas the standard path scales linearly ($\propto n$). This matters for resource-constrained IoT devices where peak RAM is the primary bottleneck.
    \item \textbf{Proving-time crossover} is far out ($\approx 2000$ items) because recursive SNARKs incur additional setup/folding/compression overhead per evaluation. As $n$ grows, the linear proving term of standard SNARKs dominates, and recursion becomes faster despite its fixed costs (see top-right panel of \cref{fig:crossover-theory}).
\end{itemize}

\noindent Together, these observations justify a \textbf{holistic threshold} at 171 items: while storage and memory advantages manifest earlier, the full \emph{overall} benefit emerges once the fixed recursive overhead is amortized and both sub-linear memory and constant final proof size contribute simultaneously.

\paragraph{Practical Implications}
Near the threshold ($n\approx 171$), the time advantage is about 30\% (ratio $\approx 1.31$). For larger $n$, the efficiency gap widens; \textit{long-running or archival IoT workloads} benefit most from recursion, whereas \textit{short, latency-critical tasks} may still prefer standard SNARKs below $\sim$100 items.
...
\label{fig:sensitivity-analysis-arch}
\end{figure}
