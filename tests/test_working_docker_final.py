#!/usr/bin/env python3
"""
✅ WORKING DOCKER FINAL TEST
Verwendet den FUNKTIONIERENDEN Ansatz vom Quick-Test
Für Items 50, 60, 70, 80, 90, 100, 110, 120
"""
...
def create_working_dockerfile():
    """Dockerfile - genau wie beim funktionierenden Test"""
...
def create_working_requirements():
    """Requirements - genau wie beim funktionierenden Test"""
...
def create_working_test_script():
    """Working Test Script - basiert auf dem funktionierenden Quick-Test"""
...
def docker_working_test(num_items, test_type):
    """Working Test - basiert auf funktionierendem Quick-Test"""
    print(f"🔬 Docker Working Test: {num_items} items ({test_type})")
    ...
    try:
        ...
        if test_type == "standard":
            ...
            for i, reading in enumerate(temp_readings):
                ...
                # Use the WORKING approach - direct generate_proof
                ...
                if result.success:
                    ...
                    # Show detailed output like original tests
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 3:
        print("Usage: python working_test.py <num_items> <test_type>")
        sys.exit(1)
    
    num_items = int(sys.argv[1])
    test_type = sys.argv[2]
    
    result = docker_working_test(num_items, test_type)
    print("\\n" + "="*80)
...
def run_working_docker_test(num_items, test_type, iot_config):
    """Führt working Test aus - basiert auf funktionierendem Quick-Test"""
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with open(dockerfile_path, 'w') as f:
            f.write(create_working_dockerfile())
        
        requirements_path = docker_project / "requirements.txt"
        with open(requirements_path, 'w') as f:
            f.write(create_working_requirements())
        
        script_path = docker_project / "working_test.py"
        with open(script_path, 'w') as f:
            f.write(create_working_test_script())
        
        # Build Docker image
        image_name = f"iot-working-{num_items}-{test_type}"
        ...
        run_cmd = [
            ...
            image_name,
            "python3", "working_test.py", str(num_items), test_type
        ]
        
        try:
            result = subprocess.run(run_cmd, capture_output=True, text=True, timeout=1800)  # 30 min timeout
            ...
            if result.returncode == 0:
                ...
                print(f"Raw output sample: {result.stdout[-500:]}")  # Show last 500 chars
                return None
            else:
            ...
            print(f"❌ Docker test timed out after 30 minutes")
            return None
        except json.JSONDecodeError as e:
            print(f"❌ Failed to parse Docker output: {e}")
            return None

def working_docker_analysis():
    """Working Docker-Analyse für Items 50-120"""
    print("✅ WORKING DOCKER FINAL ANALYSIS")
    print("Items 50, 60, 70, 80, 90, 100, 110, 120 unter IoT-Constraints")
    print("Basiert auf dem funktionierenden Quick-Test Ansatz")
    print("=" * 80)
    
    # IoT Configuration - Smart Home Hub (funktioniert)
    iot_config = {
        "name": "Smart Home Hub",
        "memory": "1g",
        "cpu": "1.0",
        "description": "Proven working IoT orchestrator configuration"
    }
    ...
    # Test items as requested
    test_items = [50, 60, 70, 80, 90, 100, 110, 120]
    all_results = []
    
    for num_items in test_items:
        print(f"\n" + "="*80)
        print(f"✅ WORKING TEST: {num_items} ITEMS")
        print("="*80)
        
        # Standard Test
        print(f"\n📊 STANDARD SNARKs TEST:")
        print("-" * 50)
        std_result = run_working_docker_test(num_items, "standard", iot_config)
        ...
        print("-" * 50)
        rec_result = run_working_docker_test(num_items, "recursive", iot_config)
...
def analyze_working_results(results):
    """Analysiert working Ergebnisse"""
    print("\n" + "="*100)
    print("📊 WORKING DOCKER ANALYSIS RESULTS")
    print("="*100)
    ...
    if crossover_point:
        ...
        print(f"   ⚠️  NO CROSSOVER found in 50-120 range under IoT constraints")
...
def main():
    """Hauptfunktion"""
    print("✅ Starting Working Docker Final Analysis...")
    
    results = working_docker_analysis()
    
    if results:
        crossover_point = analyze_working_results(results)
        
        # Save results
        results_dir = project_root / "data" / "working_docker_final"
        results_dir.mkdir(parents=True, exist_ok=True)
        
        results_file = results_dir / "working_final_results.json"
        with open(results_file, 'w') as f:
            json.dump({
                "working_final_results": results,
                "crossover_point": crossover_point,
                "test_timestamp": time.time(),
                "iot_configuration": "Smart Home Hub (1GB RAM, 1 CPU)",
                "test_items": [50, 60, 70, 80, 90, 100, 110, 120],
                "methodology": "Docker containers with realistic IoT resource constraints - working approach",
                "detailed_output": "Full pipeline output based on proven working quick test"
            }, f, indent=2)
        
        print(f"\n💾 Working final results saved: {results_file}")
        print(f"✅ WORKING DOCKER FINAL ANALYSIS COMPLETE!")
        
        return True
    else:
        print("❌ No working final results obtained")
        return False
